...
const toMysqlDate = (x) => {
      if (!x) return null;
      try {
        return new Date(x).toISOString().slice(0, 19).replace('T', ' ');
      } catch {
        return null;
      }
    };
    const cardDate = toMysqlDate(card_issued_at);
    ...
    await query(
      `UPDATE users
       SET name                = COALESCE(?, name),
           ...
           updated_at          = NOW()
       WHERE id = ?`,
      [v(name), v(nickname), v(phone), v(apartment_id), v(apartment_name), v(building), v(unit),
       v(gps_verified), v(verification_status), v(cardDate), v(phone_verified), req.user.id]
    );
