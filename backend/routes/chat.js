 ...
 * POST /api/chat/start
 * body: { request_id }
 * 글 방문자가 작성자와 채팅 시작 (작성자=requester, 나=helper)
 * 도움신청과 무관하게 바로 채팅 가능
 */
router.post('/start', requireAuth, async (req, res) => {
  try {
    ...
    if (!request_id)
      return res.status(400).json({ success: false, message: 'request_id가 필요합니다.' });

    // 글 작성자 찾기
    const [reqRows] = await pool.query('SELECT user_id FROM requests WHERE id = ?', [request_id]);
    if (reqRows.length === 0)
      return res.status(404).json({ success: false, message: '요청글을 찾을 수 없습니다.' });

    const authorId = reqRows[0].user_id;

    // 본인 글이면 채팅 불가
    if (authorId === me)
      return res.status(400).json({ success: false, message: '본인 글에는 채팅할 수 없습니다.' });

    // 이미 방이 있으면 그대로 반환 (작성자=requester, 나=helper)
    const [existing] = await pool.query(
      'SELECT id FROM chat_rooms WHERE request_id = ? AND requester_id = ? AND helper_id = ?',
      [request_id, authorId, me]
    );
    if (existing.length > 0)
      return res.json({ success: true, data: { id: existing[0].id } });
    ...
    res.status(500).json({ success: false, message: '채팅 시작 실패', error: error.message });
  }
});
