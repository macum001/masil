...
export default function ApartmentSearch() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await apartmentService.searchKakao(query);
        // 같은 이름+주소 중복 제거
        const seen = new Set();
        const unique = (res || []).filter((a) => {
  ...
  const handleNext = async () => {
    if (!selected) return;
    try {
      const reg = await apartmentService.register({
        kakao_id: selected.kakao_id,
        name: selected.name,
        address: selected.address,
        lat: selected.lat,
        lng: selected.lng,
      });
      const aptToStore = { ...selected, id: reg.id };
      sessionStorage.setItem('onboarding_apartment', JSON.stringify(aptToStore));
