...
export default function ChatRoom({ chat, onBack }) {
  ...
  const [messages, setMessages] = useState(() => initialMessages(hasGratitude));
  ...
  const handleSend = () => {
    ...
    setMessages((prev) => [...prev, { id: Date.now(), type: 'me', text, time }]);
    setInput('');
  };
      ...
      <header className="flex-shrink-0 bg-background/90 backdrop-blur-xl border-b border-border/40 z-20">
        <div className="flex items-center gap-3 px-4 pt-5 pb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[16px] font-bold text-foreground tracking-tight truncate">{chat.name}</p>
            <p className="text-[12px] text-muted-foreground truncate">{chat.postTitle}</p>
          </div>
          <button
