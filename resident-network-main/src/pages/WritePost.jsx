      ...
      <header className="sticky top-0 z-30 bg-background/90 backdrop-blur-xl border-b border-border/40">
        <div className="flex items-center gap-3 px-5 pt-5 pb-4">
          ...
          <div className="flex-1">
            <p className="text-[12px] font-semibold text-primary tracking-widest uppercase">{typeLabel}</p>
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                style={form.category === cat
                  ...
                  : { background: '#F2F2F7', color: '#8A8A8E' }
                }
              >
                {cat}
              </button>
              ...
              {GRATITUDE_OPTIONS.map((opt) => (
                <button
                  ...
                  style={form.gratitude === opt.id
                    ? { background: 'rgba(255,138,61,0.12)', border: '1.5px solid #FF8A3D' }
                    : { background: '#F2F2F7', border: '1.5px solid transparent' }
                  }
                >
                  <span
                    className="text-[15px] font-bold"
                    style={{ color: form.gratitude === opt.id ? '#FF8A3D' : '#1C1C1E' }}
                  >
                    {opt.label}
                  </span>
