...
function mapRequest(row) {
  return {
    ...
    location: row.location_text || row.location || '우리 단지',
    locationRange: row.location_range,
    time: row.preferred_time,
    timeAgo: row.time_ago || '방금 전',
    gratitude: row.gratitude_type,
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      ...
      try {
        ...
        if (mounted) setPostsError(error.message || '요청글을 불러오지 못했습니다.');
      } finally {
  ...
  const handleSubmit = async (formData) => {
    const tempPost = {
      ...
      locationRange: formData.location,
      time: formData.time,
      timeAgo: '방금 전',
    ...
    try {
      const saved = await requestService.create(formData);
      setPosts((prev) => [mapRequest(saved), ...prev.filter((p) => p.id !== tempPost.id)]);
    } catch (error) {
