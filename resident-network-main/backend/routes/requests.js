...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT r.*,
              u.nickname, u.name, u.apartment_name
       FROM requests r
...
router.patch('/:id/status', requireAuth, async (req, res) => {
  try {
    ...
    const updateResult = await query(
      'UPDATE requests SET status = ?, updated_at = NOW() WHERE id = ? AND user_id = ?',
    ...
    // done 상태가 되면 도움 준 사람의 help_count, 요청자 thanks_count 증가
    if (status === 'done') {
      // 이 요청의 helper 찾기
      const helperRow = await query(
        'SELECT helper_user_id FROM request_applications WHERE request_id = ? AND status = "applied" LIMIT 1',
        [req.params.id]
      );
      if (helperRow.rows.length > 0) {
        const helperId = helperRow.rows[0].helper_user_id;
        // helper help_count +1, manner_temp +0.1
        await query(
          'UPDATE users SET help_count = help_count + 1, manner_temp = LEAST(manner_temp + 0.1, 99.9), updated_at = NOW() WHERE id = ?',
          [helperId]
        );
        // 요청자 thanks_count +1
        await query(
          'UPDATE users SET thanks_count = thanks_count + 1, updated_at = NOW() WHERE id = ?',
          [req.user.id]
        );
      }
    }
